...
def admin_login(request):
    try:
        user = Admin.objects.get(mail=request.data['mail'], password=request.data['password'])
        user_serializer = AdminSerializer(instance=user)
        return Response(user_serializer.data, status=status.HTTP_200_OK)
...
def update_admin(request, id):
    try:
        ...
        serializer = AdminSerializer(instance=user, data=request.data)
...
def end_reservation(request):
    try:
        ...
        serializer = EndReservationSerializer(instance=reservation, data=request.data)

        if serializer.is_valid():
            serializer.save()
            time=reservation.end_time-reservation.start_time
